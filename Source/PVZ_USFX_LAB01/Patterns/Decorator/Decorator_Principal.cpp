...
void ADecorator_Principal::BeginPlay()
{
	...
	AEnemigoADistacia* EnemigoADistancia = GetWorld()->SpawnActor<AEnemigoADistacia>(AEnemigoADistacia::StaticClass());
	EnemigoADistancia->DefinirEnemigo(EnemigoCuerpoACuerpo);

	GEngine->AddOnScreenDebugMessage(-1, 15.f, FColor::Magenta, TEXT("Enemigos cuerpo a cuerpo en el horizonte"));
	Enemigo = EnemigoCuerpoACuerpo;
	Enemigo->Pelear();
	GEngine->AddOnScreenDebugMessage(-1, 15.f, FColor::Orange, FString::Printf(TEXT("Los enemigos cuerpo a cuerpo hacen %i de dano."), Enemigo->GetDano()));
	Enemigo->Morir();

	GEngine->AddOnScreenDebugMessage(-1, 15.f, FColor::Black, TEXT("Los enemigos estan armados con pistolas"));
	Enemigo = EnemigoADistancia;
	Enemigo->Pelear();
	GEngine->AddOnScreenDebugMessage(-1, 15.f, FColor::Cyan, FString::Printf(TEXT("Los enemigos a distancia hacen %i de dano."), Enemigo->GetDano()));
	Enemigo->Morir();
